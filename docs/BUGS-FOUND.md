...
This is the point of the whole repo: a test suite is only valuable if it _fails when the code is wrong_.
...
IDOR-style vulnerability. This is exactly why the access-control test asserts a 403 _and_ that the
...
**Change:** removed the calendar-validity guard, so only the `YYYY-MM-DD` _format_ was checked.
